...
app.post('/notifications', function(req, res) {
  ...
  subscribersCollection.find(params, { 'subscriber_id': true, '_id': false }).toArray(function(err, data) {
    ...
    body.subscribers = subscribers.map(function(subscriber) { return subscriber.subscriber_id });
...
app.get('/notifications', function(req, res) {
  ...
  notificationsCollection.find({ subscribers: subscriber_id }, { subscribers : false }).toArray(function(err, data) {
    if (err) return console.error(err);
    ...
    subscribersCollection.update({'subscriber_id' : subscriber_id}, { 'read' : { '$inc' : resp.length} }, function(err, data){
      res.send(resp);
    });
